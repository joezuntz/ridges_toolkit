...
def plot_shear_catalogue(catalogue, nside, title='', lon=[0,50], lat=[0,50], chunk_size=2_000_000, save_name=''):
    ...
    with h5py.File(catalogue, 'r') as f:
        ...
        for start in range(0, N, chunk_size):
            ...
            ra  = f['ra'][start:end]
            dec = f['dec'][start:end]
            gamma1 = f['g1'][start:end]
            gamma2 = f['g2'][start:end]
            ...
            gamma1_map_rec += np.bincount(pix, weights=gamma1, minlength=len(gamma1_map_rec))
            gamma2_map_rec += np.bincount(pix, weights=gamma2, minlength=len(gamma2_map_rec))
